...
alter table public.experiments
  add constraint experiments_draft_revision_check
...
create function private.update_draft_experiment(
  p_operation_id uuid,
...
begin
  ...
  if not found
    or v_experiment.lifecycle_status <> 'draft'
...
set search_path = ''
as $$
  select private.update_draft_experiment(
    p_operation_id,
    p_experiment_id,
    p_expected_revision,
    p_name,
...
from public, anon, authenticated, service_role;
revoke all on function public.update_draft_experiment(uuid, uuid, text, text, text)
from public, anon, authenticated, service_role;

grant execute on function private.update_draft_experiment(uuid, uuid, text, text, text)
to authenticated, service_role;
grant execute on function public.update_draft_experiment(uuid, uuid, text, text, text)
to authenticated, service_role;
