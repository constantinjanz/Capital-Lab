...
The checked-in Alpaca code is a data-only latest-quote and historical-bar adapter skeleton restricted to `https://data.alpaca.markets`; it has no brokerage client or order-forwarding endpoint. Initial hosted market setup creates a disabled `alpaca_iex` reference source; later universe-only reconfiguration preserves existing reviewed lifecycle state. Neither path needs credentials, makes provider requests, or changes activation. Keep `MARKET_DATA_PROVIDER=mock` until the separate ingestion and adapter-hardening review is complete.
