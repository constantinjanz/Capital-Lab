...
The hosted `/markets` page calls one owner-only database function that materializes the current universe, its current members, configured market-data source IDs, quotes/completed bars, exchange sessions, and source health in one PostgreSQL statement snapshot. The statement records its own decision timestamp, so configuration and evidence cannot mix MVCC versions. Mutable instrument, exchange, source, or calendar-source metadata is additionally rejected when it is not eligible at the boundary.
