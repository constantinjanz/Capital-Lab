...
- Protected live Supabase authentication is wired and the confirmed identity is bound as the singleton owner. Application registration/bootstrap is disabled after provisioning, but project-level public signup must also be disabled in the hosted Supabase Auth settings before production. In Supabase mode, the shell, dashboard landing state, experiment/control list, and experiment detail use owner-scoped hosted reads and never fall back to fixtures. The detail view preserves numeric and bigint values as exact strings. Portfolio, market, cost, remaining feature pages, and interactive lifecycle writes stay mock/read-only until their precision-safe views and reviewed write use cases replace them.
