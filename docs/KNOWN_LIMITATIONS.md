...
- Protected live Supabase authentication and confirmed-email singleton bootstrap are wired, but the pre-authorized person must still create and confirm the first hosted account. In Supabase mode, the shell, dashboard landing state, and experiment/control list use owner-scoped hosted reads and never fall back to fixtures. Portfolio, market, cost, remaining feature pages, and interactive lifecycle writes stay mock/read-only until precision-safe decimal-string views and reviewed write use cases replace them.
