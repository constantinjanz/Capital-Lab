...
5. Keep `MARKET_DATA_PROVIDER=mock` and omit Alpaca credentials while the checked-in adapter remains an unhardened, unpersisted skeleton. Enabling Alpaca requires a separate reviewed ingestion slice with raw/as-of semantics, pagination, request bounds, sanitized failures, and a passing safety scan.
